...
# added by Anaconda3 installer, edit <user>
...
# extract files by automatically detecting file extension
extract () {
  if [ -f $1 ] ; then
    case $1 in
      *.tar.bz2)   tar xvjf $1    ;;
...
# list number of files in a folder
numfiles() { 
  if [ -z "$1" ]; then
    N="$(ls | wc -l)"; 
    echo "$N files";
...
# list number of files in a folder and all of its subfolders
numfiles-all() { 
  if [ -z "$1" ]; then
    N="$(find . -type f | wc -l)";
    echo "$N files";
